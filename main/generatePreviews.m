...
for i = 1:app.numFrames
    ...
    if isempty(find(app.ignoreFrames == index, 1))
        ...
        uiimage(app.MaskOverlayPreviewPanel, "ScaleMethod",'scaledown', 'ImageSource', ...
            labeloverlay(frames(:, :, index), mask(:, :, index)), "Position", [5, (5 + (i - 1)*(height/width*panelPos(4))), panelPos(3) - 10, height/width*panelPos(4)], ...
            ...
            image = hsv2rgb(image);
            uiimage(app.MaskOverlayPreviewPanel, "ScaleMethod",'scaledown', 'ImageSource', ...
                image, "Position", [10, (10 + (i - 1)*(310./395.*panelPos(4))), panelPos(3) - 20, 310./395.*panelPos(4)], ...
                "ImageClickedFcn", {@imageClicked, app},'Tooltip', "Click to ignore frame " + num2str(index) + " during calculations", 'Tag', num2str(index)...
                ,"ContextMenu", contextMenu);
        end
    else
        ...
        if any(any(mask(:, :, index)))
        uiimage(app.MaskOverlayPreviewPanel, "ScaleMethod",'scaledown', 'ImageSource', ...
            labeloverlay(frames(:, :, index), ones(size(frames(:, :, index))), 'Colormap', 'autumn'), "Position", [10, (10 + (i - 1)*(310./395.*panelPos(4))), panelPos(3) - 20, 310./395.*panelPos(4)], ...
            ...
            image = hsv2rgb(image);
            uiimage(app.MaskOverlayPreviewPanel, "ScaleMethod",'scaledown', 'ImageSource', ...
                labeloverlay(image, ones(size(frames(:, :, index))), 'Colormap', 'autumn'), "Position", [10, (10 + (i - 1)*(310./395.*panelPos(4))), panelPos(3) - 20, 310./395.*panelPos(4)], ...
                "ImageClickedFcn", {@imageClicked, app},'Tooltip', "This frame will be ignored during bubble analysis", 'Tag', num2str(index) + "NaN"...
                ,"ContextMenu", contextMenu);
        end
    end
